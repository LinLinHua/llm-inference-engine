...
Problem without paging:
  - Each request needs contiguous memory for its KV cache
  - Unknown sequence lengths → must pre-allocate max_len → huge waste
  - 100 requests × max_len=4096 × 2 layers × 8 heads × 128 dim × 2 bytes = 3.2GB
  - Most of it wasted (internal fragmentation)

Solution (vLLM PagedAttention):
  - Split KV cache into fixed-size blocks (like OS virtual memory)
  - Allocate blocks on demand, free when request finishes
  - Near-zero fragmentation → more concurrent requests

Physical layout (vLLM-compatible):
  key_cache[layer]:   [num_blocks, num_kv_heads, head_dim, block_size]
  value_cache[layer]: [num_blocks, num_kv_heads, head_dim, block_size]
...
class PagedKVCache:
    def __init__(
        ...
        # Physical storage for all layers
        ...
        # Block allocator: stack of free physical block IDs
